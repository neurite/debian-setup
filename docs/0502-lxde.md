...
Even though [Lean LXDE](docs/0502-lxde.md) and [Lean GNOME](docs/0501-gnome.md) both install a basic desktop, keep in mind that LXDE is much lighter than GNOME. Choose LXDE if you want a super minimal desktop, if you don't mind manually installing and setting up additional packages on top of LXDE (which can be challenging), and if you don't care much about the visual appeal. On the other hand, GNOME provides many of the packages such as pulse audio, a display manager, a wireless network manager, a PDF viewer right out of the box. You will find that most of them will be needed at some point and need to be installed anyways.

Before you start, **make sure you don't already have a desktop installed**. Follow instructions on [Manual Install](docs/0200-install.md). There, when you choose "Expert Install", you will have the option to not to install any default desktop environment. This allows us to install a bare minimum desktop based on LXDE here.
