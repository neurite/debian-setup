...
10. `Set up users and passwords`. `Allow login as root`: `No`. Forces the use of `sudo`.
11. `Configure the clock`. * `Set the clock using NTP`: `Yes`.
12. `Detect disks`.
13. `Partition disks`.
